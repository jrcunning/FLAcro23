...
```{r}
sr_colors <- c('#1b9e77','#d95f02','#7570b3','#e7298a','#66a61e','#e6ab02')
sr_colors <- unname(graf_palettes$bright[1:6])
sr_colors <- unname(graf_palettes$fishy[1:6])
# Determine size breaks for legend
size_breaks <- c(10, 100, 1000, 5000)  # example breaks
size_labels <- size_breaks

mapfig2 <- basemap +
  geom_sf(data = world, color = "black", lwd = 0.1) +
  geom_rect(
    aes(xmin = -79.5, xmax = -79, ymin = 25.5, ymax = 26),
    fill = "white", color = NA
  ) +
  geom_rect(
    aes(xmin = -79.7, xmax = -79.2, ymin = 23.5, ymax = 23.6),
    fill = "white", color = NA
  ) +
  ...
  coord_sf(xlim = c(-83.45, -78.95), ylim = c(23.5, 26.4), expand = FALSE) +
...
bars <- out.glmer.nospp %>%
  ...
  mutate(barplots = map(data, ~ggplot(., aes(x = factor(date))) +
             ...
             geom_text(aes(x = 1.5, y = 1.1, label = region_label, color = subregion2),
...
outmapbars <- ggdraw() +
  draw_plot(mapfig2) +
  draw_plot(bars$barplots[[1]], height = 0.24, width = 0.15, x = 0.775, y = 0.69) +
  draw_plot(bars$barplots[[2]], height = 0.24, width = 0.15, x = 0.775, y = 0.45) +
  draw_plot(bars$barplots[[3]], height = 0.24, width = 0.15, x = 0.68, y = 0.22) +
  draw_plot(bars$barplots[[4]], height = 0.24, width = 0.15, x = 0.52, y = 0.075) +
  draw_plot(bars$barplots[[5]], height = 0.24, width = 0.15, x = 0.32, y = 0.03) +
  draw_plot(bars$barplots[[6]], height = 0.24, width = 0.15, x = 0.1, y = 0.05)

# Display map figure with additional plots
outmap3 <- outmapbars +
  draw_plot(dhwfig, height = 0.4, width = 0.35, x = 0.1, y = 0.52)



# CUSTOM LEGEND
# 1️⃣ Compute size ranges
size_data_range <- range(sqrt(dfs2$maxn + 20))
size_range <- c(0.5, 8)

# 2️⃣ Create legend data (2 rows, 4 columns)
...
# 3️⃣ Compute dynamic column widths
...
# 4️⃣ Merge dynamic x-positions back to legend_data
legend_data <- legend_data %>%
  left_join(col_widths %>% dplyr::select(maxn, x_final), by = "maxn") %>%
  mutate(x = x_final)

# 5️⃣ Compute row heights (like before for y)
...
# 6️⃣ Merge dynamic y-positions back to legend_data
legend_data <- legend_data %>%
  left_join(row_heights %>% dplyr::select(species, y_final), by = "species") %>%
  mutate(y = y_final)

# 7️⃣ Build the transposed legend matrix plot
...
# Display the final legend matrix plot
#legend_matrix_plot


# Show the legend matrix plot
#legend_matrix_plot
...
(outmap4 <- outmap3 + draw_plot(legend_matrix_plot, height = 0.13, width = 0.13, x = 0.84, y = 0.08))

# hist(df$n_total)
# summary(df$n_total)

# Save
ggsave("outputs/Fig3a.png", plot = outmap4, width = 183, height = 130.7, units = "mm")
#ggsave("outputs/Fig3.pdf", plot = outmap4, width = 183, height = 130.7, units = "mm")
