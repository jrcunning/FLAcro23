...
### No significant differences (at p=0.01) between species in any subregion on either date
### Note almost significant difference in Biscayne on March 1st 2024 (p=0.0194); however, estimates for ACER on that date
...
```{r}
# Get results from glmer for BOTH species combined
res.glmer <- emmeans(mod.glmer, specs = c("date_scaled", "subregion"), type = "response",
               at = list(date_scaled = seq(-2.168304, 2.380493, 0.014)))

# Backtransform scaled dates to actual dates
res.glmer <- as.tibble(res.glmer) %>% mutate(date = as_date(date_scaled * datesd + datemean))

# Pull out Oct1 and Mar1 mortality for each region
out.glmer <- res.glmer %>%
