...
```{r}
survivors <- df %>%
  filter(date > "2024-03-01", pct_mort < 0.5) %>%
...
surv_est <- glmer_fitted_mort %>%
  filter(date == "2024-03-01") %>%
  left_join(counts1.byspp) %>%
  mutate(surviv_tot = round(maxtot1 * (1 - prob), 0)) %>%
  dplyr::select(subregion, species, mort = prob, n_obs = maxtot1, n_alive = surviv_tot)

surv_est

surv_est %>%
  mutate(mort = sprintf("%.4f", mort))

surv_est <- surv_est %>%
  mutate(
    mort_percent = ifelse(
      abs(mort - 1) < 1e-8,  # treat anything within 1e-8 of 1 as exactly 1
      "100.00%",
      paste0(floor(mort * 10000) / 100, "%")
    )
  )

surv_est <- surv_est %>%
  dplyr::select(subregion, species, n_obs, mort_percent, n_alive)

surv_est %>%
  mutate(pct = n_alive / sum(n_alive)) %>%
  group_by(subregion) %>%
  summarize(totpct = sum(pct))

write_csv(surv_est, file = "outputs/surv_est.csv")

surv_est %>% arrange(subregion)


#full_join(glmer_fitted_mort, out.glmer.nospp, by = c("subregion", "date"))
...
tables2 <- bind_rows(spp = glmer_fitted_mort, both = out.glmer.nospp, .id = "mod") %>%
  ...
  pivot_wider(names_from = c(date, species), values_from = prob)


tables2 <- counts1.byspp %>%
  pivot_wider(names_from = species, values_from = maxtot1) %>%
  full_join(tables2)
