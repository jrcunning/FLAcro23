...
# Response differences between APAL and ACER - DELETE?
...
# Test wild vs. outplant
# Fit glmer model with wild/outplant as factor
dff <- df %>% filter(coral_type != "Wild/Outplant")
mod.glmer.f <- glmer(cbind(n_dead, n_alive) ~ subregion * date_scaled * species * coral_type + (1| siteclust/species),
                   family = binomial,
                   data = dff)
# On October 1st, 2023
emm1001 <- emmeans(mod.glmer.f, specs = "coral_type", by = c("date_scaled", "subregion", "species"),
               at = list(date_scaled = as.numeric(as.Date("2023-10-01"))))
rbind(contrast(emm1001, "pairwise"))
# On March 1st, 2024
emm0301 <- emmeans(mod.glmer.f, specs = "coral_type", by = c("date_scaled", "subregion", "species"),
               at = list(date_scaled = as.numeric(as.Date("2024-03-01"))))
rbind(contrast(emm0301, "pairwise"))
...
mod.dhw.glmer <- glmer(cbind(n_dead, n_alive) ~ cummaxdhw * species * coral_type + (1|subregion),# (1|siteclust/species),
                   family = binomial,
                   data = dff)
car::Anova(mod.dhw.glmer)
#predict data for GLMM
dhwGlmerRes <- emmeans(mod.dhw.glmer, specs = "cummaxdhw", by = c("species", "coral_type"), type = "response", interval = "prediction",
               at = list(cummaxdhw = seq(0, 25, 0.1))) %>%
  as_tibble(.)%>%
  mutate(type="glmer")
...
```{r}
#estimate ed50s/ed95s comebine predicted values
...
#calculate ed50s from package
library(ed50)
estimatesEd50 <- resDhwMods%>%
  group_by(type, coral_type, species) %>%
...
ed50s <- ed50s %>% unnest(ed50)


#estimate ed95 through some math. Could get closer with different intervals
ed95 <- resDhwMods%>%
  mutate(closeness=abs(0.95-prob))%>%
  group_by(type, coral_type, species)%>%
  slice_min(closeness)


# plot Just fitted curves with labels for each ed value by species
library(ggrepel)
resDhwMods %>%
  ggplot(aes(x = cummaxdhw, y = prob, lty = type)) +
  facet_wrap(~species + coral_type) +
...
```{r plot just glmm}
#plot just GLMM values for each species 
library(ggtext)
resDhwMods %>%
  filter(type=="glmer")%>%
  ggplot(aes(x = cummaxdhw, y = prob, color = species)) +
  facet_wrap(~coral_type + species) +
  geom_ribbon(aes(ymin = asymp.LCL, ymax = asymp.UCL), alpha = 0.15, lwd = 0) +
  geom_line(size=1.2) +
  ...
  theme(legend.position = "none", legend.text = element_markdown())
