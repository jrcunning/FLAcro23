...
# Combine all data, cluster sites, assign subregions
```{r}
df <- bind_rows(
...
clusters <- cutree(hc, h = 2000)  # Cluster based on 1000m threshold
...
```{r}
df <- df %>%
  mutate(n_total_cat = cut(n_total, breaks = c(0, 10, 100, 500, 1000)))
```
...
# Combine Mid-Upper Keys Transition with Middle Keys (only ~2 timepoints) with Lower Keys
df <- df %>%
