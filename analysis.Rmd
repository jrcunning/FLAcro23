...
```{r}
emm <- emmeans(mod, specs = "species", by = c("date", "subregion"), type = "response",
               at = list(date = as.Date("2023-10-01")))
contrast(emm, "pairwise")

# Sig. diffs in Upper, Middle, and Lower Keys, but small diffs
# Acer vs. Apal
# Upper Keys  75% vs. 79%
# Middle Keys 100% vs. 96%
# Lower Keys 95% vs. 98%
...
mod2 <- glm(cbind(n_dead, n_alive) ~ subregion * date, family = "binomial", data = df)
...
```{r}
res2 <- emmeans(mod2, specs = c("date", "subregion"), type = "response",
                at = list(date = seq(as.Date("2023-03-14"), as.Date("2024-06-01"), 
                                    by = "day")))
# Filter fitted values to just the range of observed data
date_ranges <- df %>%
...
# Pull out Oct1 and Apr1 mortality for each region
as.tibble(res2.f) %>%
  filter(date %in% c("2023-10-01", "2024-04-01")) %>%
